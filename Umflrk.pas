...
uses UddSel, UDM, UGlobal, Urkjm;

{$R *.dfm}

procedure TFmflrk.RzButtonEdit1ButtonClick(Sender: TObject);
begin
  inherited;
...
procedure TFmflrk.RzBitBtn1Click(Sender: TObject);
begin
  inherited;
...
procedure TFmflrk.cxGrid1DBTableView1CellClick(
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select * from V_mflrk where jhbsid='+QuotedStr(ADOQuery1.FieldByName('id').AsString));
    Open;
...
procedure TFmflrk.FormShow(Sender: TObject);
begin
  inherited;
  BitBtn1.Enabled:=False;
end;

procedure TFmflrk.BitBtn1Click(Sender: TObject);
begin
  inherited;
  ...
  Frkjm:=TFrkjm.Create(Application);
  Frkjm.ShowModal;
  Frkjm.Free;
end;
